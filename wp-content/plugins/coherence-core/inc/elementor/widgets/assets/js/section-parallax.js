	...
	$(window).on('elementor/frontend/init', function () {
		...
		let FrontEndExtended = elementorModules.frontend.handlers.Base.extend({
			...
			generateParallaxConfig: function() {
				var parallaxConfig = {};
				if(this.getElementSettings('coherence_parallax_activate') === 'yes') {
					parallaxConfig['parallax'] = true;
					parallaxConfig['mobile'] = this.getElementSettings('coherence_parallax_activate_mobile') === 'yes';
					parallaxConfig['type'] = this.getElementSettings('coherence_parallax_type') ? this.getElementSettings('coherence_parallax_type') : 'vertical';
				}
			...
			onInit: function () {
				...
				if(!editMode || coherence_section_parallax[this.getID()] != undefined) {
					parallaxConfig = coherence_section_parallax[this.getID()] || false;
				} else {
					parallaxConfig = this.generateParallaxConfig();
				}
				if (parallaxConfig) {
					$( '.coherence-section-parallax-background', this.$element ).remove();
					var $backgroundWrap = $('<div class="coherence-section-parallax"></div>');
					var $background = $('<div class="coherence-section-parallax-background"></div>');
					...
					$backgroundWrap.addClass('coherence-section-parallax-' + parallax_type);
					...
					if (!window.gemSettings.isTouch || mobile_enabled === '1') {
						if ($backgroundWrap.hasClass('coherence-section-parallax-vertical')) {
							var parallaxOptions = {};
							if (is_custom_title) {
						...
						} else if ($backgroundWrap.hasClass('coherence-section-parallax-horizontal')) {
		...
		if (window.gemSettings.parallaxDisabled) {
			...
			link.rel  = 'stylesheet';
			link.type = 'text/css';
			link.innerHTML = ".coherence-section-parallax.coherence-section-parallax-fixed .coherence-section-parallax-background { background-attachment: scroll !important; }";
