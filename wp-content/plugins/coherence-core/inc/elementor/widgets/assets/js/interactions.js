		...
		let FrontEndExtended = elementorModules.frontend.handlers.Base.extend({
			...
			generateInteractionsConfig: function () {
				...
				if (this.getElementSettings('coherence_interaction_vertical_scroll') === 'yes') {
					...
					this.interactionsConfig['vertical_scroll_speed'] = this.getElementSettings('coherence_interaction_vertical_scroll_speed')['size']; //From 0 to 10
			...
			onInit: function () {
				...
				if (!editMode || coherence_interactions[this.getID()] !== undefined) {
					this.interactionsConfig = coherence_interactions[this.getID()] || false;
				} else {
					this.generateInteractionsConfig();
				}
				if (this.interactionsConfig) {
					...
					elementorFrontend.elements.$window.on('load', function (e) {
						if (element.$element.hasClass('coherence-interaction-scroll')) {
